...
func ConvertError(err error) commonErrors.Error {
	...
	return commonErrors.NewError(SmsConvertErr, fmt.Sprintf("cannot convert error(%v) to Error", err.Error()))
}
...
func ConvertRPCError(err error) commonErrors.Error {
	...
	if !ok {
		return commonErrors.NewError(SmsConvertErr, fmt.Sprintf("cannot convert rpc error(%v) to Error", err.Error()))
	}
